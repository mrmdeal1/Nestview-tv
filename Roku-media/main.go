...
const (
	...
	ptsOffset int64 = 1800000 // V86: 20s on 90kHz clock; align first Nest PTS with end of Apple bootstrap segment
)
...
var (
	...
	v83BlockMu   sync.RWMutex
	v83NestBlock []HLSSegment
	v83StartSequence int
)
...
func buildFrozenVOD(session *StreamSession) (int, float64) {
	...
	log.Printf(
		"VERSION 86 FROZEN VOD READY: camera=%d name=%s generation=%d segments=%d duration=%.3f codec=[%s]",
		cameraIndex,
		cameraName,
		generation,
		len(copied),
...
func getCameras() ([]Camera, error) {
	...
	for attempt := 1; attempt <= 3; attempt++ {
		log.Printf(
			"VERSION 86 camera backend request attempt=%d url=%s/api/cameras",
			attempt,
			nestBackend,
		)
		...
		if err != nil {
			lastErr = err
			log.Printf(
				"VERSION 86 camera backend request failed attempt=%d error=%v",
				attempt,
				err,
			)
		} else {
			...
			if readErr != nil {
				lastErr = readErr
				log.Printf(
					"VERSION 86 camera backend read failed attempt=%d error=%v",
					attempt,
					readErr,
				)
				...
				log.Printf(
					"VERSION 86 camera backend HTTP failure attempt=%d status=%d body=%q",
					attempt,
					resp.StatusCode,
					string(responseBody),
				)
				...
				log.Printf(
					"VERSION 86 camera backend success attempt=%d status=%d bytes=%d",
					attempt,
					resp.StatusCode,
					len(body),
				)
...
func (s *StreamSession) inspectSliceOrderingLocked(
	...
	for _, nalu := range nalus {
		...
		if !diagnostic.Valid {
			...
			log.Printf(
				"VERSION 86 H264 SLICE PARSE ERROR: NAL=%d PTS=%d error=%s",
				nalType,
				pts,
				diagnostic.Error,
			)
		...
		if diagnostic.SliceType == "B" ||
			...
			log.Printf(
				"VERSION 86 H264 REORDER SIGNAL: slice=%s frameNum=%d POC=%d previousPOC=%d POCBackward=%t IDR=%t PTS=%d",
...
func (s *StreamSession) updateH264DiagnosticsLocked(
	...
	if !diagnostics.Valid {
		if hasSPS {
			log.Printf(
				"VERSION 86 H264 SPS PARSE ERROR: %s",
				diagnostics.Error,
			)
		}
	...
	if !s.haveActiveCodec {
		...
		log.Printf(
			"VERSION 86 H264 INITIAL CODEC: %s profileName=%s levelName=%s SPS=%d PPS=%d IDR=%t PTS=%d",
	...
	log.Printf(
		"VERSION 86 CODEC CHANGE DETECTED #%d: FROM [%s] TO [%s] IDR=%t PTS=%d",
		changeNumber,
	...
	if s.segmentActive &&
		...
		log.Printf(
			"VERSION 86 closing old codec segment before SPS transition: sequence=%d generation=%d",
			s.NextSequence,
			s.currentGeneration,
		)
	...
	log.Printf(
		"VERSION 86 HLS DISCONTINUITY ARMED: generation=%d nextSequence=%d",
		s.currentGeneration,
		s.NextSequence,
	)
}
...
func (s *StreamSession) normalizeTimestamp(
	...
	if !s.timestampStarted {
		...
		log.Printf(
			"VERSION 86 timestamp clock started: RTP=%d PTS=%d offsetSeconds=120",
			rtpTimestamp,
			s.normalizedPTS,
		)
	...
	if delta > 90000*10 {
		...
		log.Printf(
			"VERSION 86 timestamp discontinuity: previous=%d current=%d rawDelta=%d",
			s.lastRTPTimestamp,
			rtpTimestamp,
			delta,
		)
...
func (s *StreamSession) newSegmentLocked() error {
	...
	log.Printf(
		"VERSION 86 SEGMENT OPEN: sequence=%d generation=%d codec=[%s] discontinuity=%t",
		s.NextSequence,
		s.currentGeneration,
...
func (s *StreamSession) finishSegmentLocked() {
	...
	if valid {
		...
		log.Printf(
			"VERSION 86 TS MEDIA VALID: packets=%d bytes=%d PAT=%d PMT=%d videoPID=%d streamType=0x%02x PES=%d PTS=%d DTS=%d PCR=%d continuityErrors=%d firstPTS=%d lastPTS=%d",
			diagnostics.Packets,
			diagnostics.Bytes,
		...
		log.Printf(
			"VERSION 86 TS MEDIA INVALID: error=%s packets=%d PAT=%d PMT=%d videoPID=%d streamType=0x%02x PES=%d PTS=%d backwardPTS=%d PCR=%d continuityErrors=%d transportErrors=%d",
			validateErr,
			diagnostics.Packets,
			diagnostics.PATPackets,
			diagnostics.PMTPackets,
	...
	log.Printf(
		"VERSION 86 HLS SEGMENT READY: sequence=%d generation=%d duration=%.3f size=%d mediaValid=%t discontinuityBefore=%t codec=[%s]",
		segment.Sequence,
		segment.Generation,
		segment.Duration,
		len(segment.Data),
...
func (s *StreamSession) keyframeAccessUnitLocked(
	...
	*/
	...
	if changed {
		log.Printf(
			"VERSION 86 H264 NORMALIZE: before=%s after=%s removedSPS=%d removedPPS=%d",
			before,
			after,
			duplicateSPS,
			duplicatePPS,
...
func (s *StreamSession) writeAccessUnit(
	...
	if !s.segmentActive {
		...
		log.Printf(
			"VERSION 86 HLS started on IDR: generation=%d SPS=%t PPS=%t PTS=%d NAL=%s codec=[%s]",
			s.currentGeneration,
			len(s.sps) > 0,
			len(s.pps) > 0,
			pts,
	...
	if hasIDR &&
		...
		log.Printf(
			"VERSION 86 new IDR segment: sequence=%d generation=%d PTS=%d NAL=%s",
			s.NextSequence,
			s.currentGeneration,
			pts,
			sourceSummary,
	...
	if hasIDR {
		log.Printf(
			"VERSION 86 MUX IDR: PTS=%d bytes=%d NAL=%s",
			pts,
			len(outputAU),
			s.lastMuxNALSummary,
		)
	...
	// VERSION 86: give Roku an explicit decode timestamp (DTS).
...
func createStreamSession(
	...
	pc.OnTrack(
		func(
			...
			log.Printf(
				"VERSION 86 incoming track: kind=%s codec=%s payload=%d",
				track.Kind().String(),
				codec.MimeType,
				codec.PayloadType,
			)
			...
			if track.Kind() == webrtc.RTPCodecTypeVideo {
				go func() {
					...
					for {
						...
						if err != nil {
							log.Printf(
								"VERSION 86 H264 depacketize error: %v",
								err,
							)
						...
						if packet.Marker &&
							...
							if err := session.writeAccessUnit(
								...
								log.Printf(
									"VERSION 86 MPEGTS write error: %v",
									err,
								)
							}
							...
							if units == 1 ||
								...
								log.Printf(
									"VERSION 86 H264 AU: units=%d packets=%d size=%d PTS=%d NAL=%s",
									units,
									packets,
									len(accessUnit),
									pts,
	...
	log.Println(
		"VERSION 86 SDP confirmed: audio -> video -> application",
	)
	...
	log.Printf(
		"VERSION 86 Nest backend HTTP status: %d",
		resp.StatusCode,
	)
	...
	log.Println(
		"VERSION 86 Nest WebRTC session started",
	)
...
func start(
	...
	log.Printf(
		"VERSION 86 start request parsed: camera=%d; requesting camera list",
		cameraIndex,
	)
	...
	if err != nil {
		log.Printf(
			"VERSION 86 start stopped at camera backend: %v",
			err,
		)
	...
	log.Printf(
		"VERSION 86 camera list ready: count=%d requested=%d",
		len(cameras),
		cameraIndex,
	)
	...
	log.Printf(
		"VERSION 86 starting camera %d: %s",
		cameraIndex,
		camera.Name,
	)
	...
	if err != nil {
		log.Println(
			"VERSION 86 camera start failed:",
			err,
		)
...
func snapshotLatestDebugSegment() (HLSSegment, int, error) {
	...
	log.Printf(
		"VERSION 86 DEBUG SEGMENT SNAPSHOT: camera=%d generation=%d sequence=%d bytes=%d",
		camera,
		segment.Generation,
		segment.Sequence,
		len(segment.Data),
...
func liveMasterPlaylistHandler(
	...
	log.Printf("VERSION 86 ROKU REQUEST STEP=APPLE_SEGMENT_TRACE method=%s path=%s ua=%q remote=%s", r.Method, r.URL.Path, r.UserAgent(), r.RemoteAddr)

	// V74 readiness gate: freeze five consecutive validated Nest segments in
	...
	for time.Now().Before(deadline) {
		...
		if session != nil {
			...
			if len(validated) >= 5 {
				readyBlock = validated[len(validated)-5:]
				break
			}
		}
	...
	if len(readyBlock) < 5 {
		log.Printf("VERSION 86 PLAYLIST GATE WAIT timedOut=true readySegments=%d", len(readyBlock))
		w.Header().Set("Retry-After", "1")
		http.Error(w, "waiting for five validated Nest segments", http.StatusServiceUnavailable)
		return
	}

	v83BlockMu.Lock()
	v83NestBlock = make([]HLSSegment, len(readyBlock))
	v83StartSequence = readyBlock[0].Sequence
	for i := range readyBlock {
		v83NestBlock[i] = copyHLSSegment(readyBlock[i])
	}
	v83BlockMu.Unlock()
	...
	log.Printf("VERSION 86 PLAYLIST GATE READY segments=5 sequences=%s generations=%s", strings.Join(seqs, ","), strings.Join(gens, ","))
	...
	// VERSION 86 continuous diagnostic: Apple remains the proven HLS shell, but
	// every media position from index 1 onward is supplied by the live Nest stream.
	// A discontinuity is advertised at the Apple-to-Nest handoff and at actual
	// Nest generation changes that are already known when the playlist is built.
	// Startup is reduced to five validated segments; segment delivery remains
	// paced to live Nest production.
	inLines := strings.Split(string(childBody), "\n")
	// V81: preserve the exact Apple media-playlist syntax Roku accepts, but extend
	// the media timeline by replaying Apple's URI/EXTINF pattern four times.
	// The proxy index is monotonic across repeats, so every slot from index 1 onward
	// maps to the next live Nest segment. ENDLIST is emitted only after the final
	// repeat, moving the old ~1:19 ceiling several minutes out without introducing
	// a new playlist grammar.
	const shellRepeats = 4
	mediaIndex := 0
	outLines := make([]string, 0, len(inLines)*shellRepeats)
	for pass := 0; pass < shellRepeats; pass++ {
		for _, line := range inLines {
			t := strings.TrimSpace(line)
			// V82: Roku must not be told this extended camera stream is fixed VOD.
			if strings.HasPrefix(t, "#EXT-X-PLAYLIST-TYPE:") { continue }
			// Playlist-wide header/control tags belong only to the first pass.
			if t == "#EXTM3U" && pass > 0 { continue }
			if strings.HasPrefix(t, "#EXT-X-VERSION:") && pass > 0 { continue }
			if strings.HasPrefix(t, "#EXT-X-TARGETDURATION:") && pass > 0 { continue }
			if strings.HasPrefix(t, "#EXT-X-MEDIA-SEQUENCE:") && pass > 0 { continue }
			if strings.HasPrefix(t, "#EXT-X-INDEPENDENT-SEGMENTS") && pass > 0 { continue }
			if t == "#EXT-X-ENDLIST" {
				if pass == shellRepeats-1 { outLines = append(outLines, line) }
				continue
			}
			if t == "" || strings.HasPrefix(t, "#") {
			...
			if mediaIndex == 1 {
				...
				// For Nest segments already buffered when this playlist is built, signal
				// only a real codec/resolution generation change.
				v83BlockMu.RLock()
				startSequence := v83StartSequence
				v83BlockMu.RUnlock()
				wantSeq := startSequence + (mediaIndex - 1)
				prevSeq := wantSeq - 1
				session := getSession()
				if session != nil {
					session.mu.RLock()
					...
					if wantFound && prevFound && wantGen != prevGen {
						outLines = append(outLines, "#EXT-X-DISCONTINUITY")
						log.Printf("VERSION 86 PLAYLIST CODEC DISCONTINUITY appleIndex=%d fromGeneration=%d toGeneration=%d", mediaIndex, prevGen, wantGen)
					}
				}
			}
	...
	log.Printf("VERSION 86 HANDOFF CLOCK ALIGN handoffIndex=1 nestPTSStart=1800000 nestPTSSeconds=20 child=%s mediaURIs=%d bytes=%d extendedShell=true repeats=4 vodTagStripped=true", childURL, mediaIndex, len(out))
...
func appleSegmentTraceHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("VERSION 86 ROKU APPLE SEGMENT REQUEST index=%s target=%s ua=%q", idx, target, r.UserAgent())

	// VERSION 86: index 1 and every later Apple slot maps to the next Nest
	// sequence. If Roku asks ahead of real time, hold the HTTP request until that
	// Nest segment exists. This paces Roku to the camera instead of letting it race
	// through the finite Apple playlist.
	idxNum, idxErr := strconv.Atoi(idx)
	if idxErr == nil && idxNum >= 1 {
		v83BlockMu.RLock()
		startSequence := v83StartSequence
		v83BlockMu.RUnlock()
		targetSequence := startSequence + (idxNum - 1)
		...
		for time.Now().Before(deadline) {
			...
			if session != nil {
				...
				for i := range session.Segments {
					...
					if seg.Sequence == targetSequence && seg.Validated && len(seg.Data) > 0 {
						copySeg := copyHLSSegment(seg)
						found = &copySeg
						break
					}
				}
				...
				if found != nil {
					...
					diag := diagnoseTS(body)
					rb := readbackTS(body)
					log.Printf("VERSION 86 CONTINUOUS NEST SEGMENT appleIndex=%d nestSequence=%d generation=%d duration=%.3f bytes=%d", idxNum, found.Sequence, found.Generation, found.Duration, len(body))
					log.Printf("VERSION 86 TS CLOCK appleIndex=%d nestSequence=%d generation=%d firstPTS=%d lastPTS=%d firstDTS=%d lastDTS=%d firstPCR=%d lastPCR=%d ptsCount=%d dtsCount=%d pcrPackets=%d backwardPTS=%d continuityErrors=%d readbackOK=%t accessUnits=%d",
						idxNum, found.Sequence, found.Generation,
						diag.FirstPTS, diag.LastPTS, rb.FirstDTS, rb.LastDTS,
						diag.FirstPCR, diag.LastPCR, diag.PTSCount, diag.DTSCount,
						diag.PCRPackets, diag.BackwardPTS, diag.ContinuityErrors,
						rb.Success, rb.AccessUnits)
					_, _ = w.Write(body)
					return
				}
			}
		...
		log.Printf("VERSION 86 CONTINUOUS WAIT TIMEOUT appleIndex=%d nestSequence=%d", idxNum, targetSequence)
		w.Header().Set("Retry-After", "1")
	...
	w.Header().Set("Cache-Control", "no-store")
	w.Header().Set("Content-Length", strconv.Itoa(len(body)))
	log.Printf("VERSION 86 APPLE SEGMENT SERVED index=%s bytes=%d type=%q", idx, len(body), ct)
	_, _ = w.Write(body)
}
...
func livePlaylistHandler(
	...
	log.Printf("VERSION 86 ROKU REQUEST STEP=MEDIA method=%s path=%s query=%q ua=%q remote=%s", r.Method, r.URL.Path, r.URL.RawQuery, r.UserAgent(), r.RemoteAddr)
	session := getSession()
	...
	// VERSION 86: Roku startup buffer guard. Prefer the newest codec
	...
	log.Printf(
		"VERSION 86 ROKU STARTUP WINDOW: latestGeneration=%d latestCount=%d selectedGeneration=%d selectedCount=%d minimum=%d",
		latestGeneration,
	...
	// VERSION 86: Never advertise an undersized startup playlist to Roku.
	...
	if len(filtered) < rokuStartupSegments {
		log.Printf(
			"VERSION 86 ROKU MEDIA NOT READY: generation=%d ready=%d required=%d returning=503",
			selectedGeneration,
			len(filtered),
			rokuStartupSegments,
		)
	...
	if len(filtered) != len(segments) {
		log.Printf(
			"VERSION 86 LIVE PLAYLIST GENERATION FILTER: generation=%d kept=%d dropped=%d",
			selectedGeneration,
			len(filtered),
	...
	w.Header().Set("Content-Length", strconv.Itoa(len(body)))
	log.Printf("VERSION 86 ROKU MEDIA PLAYLIST BODY bytes=%d:\n%s", len(body), body)
...
func liveSegmentHandler(
	...
	log.Printf("VERSION 86 ROKU REQUEST STEP=SEGMENT method=%s path=%s query=%q range=%q ua=%q remote=%s", r.Method, r.URL.Path, r.URL.RawQuery, r.Header.Get("Range"), r.UserAgent(), r.RemoteAddr)
	session := getSession()
...
func main() {
	log.Println(
		"NestView TV Pion Roku bridge VERSION 86 starting",
	)

	log.Println(
		"VERSION 86: camera-backend retry and start-boundary diagnostics enabled",
	)
	...
	// VERSION 86: keep V56's proven HTTP redirect path, but redirect Roku
