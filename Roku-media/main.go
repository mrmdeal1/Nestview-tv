...
var (
	...
	v79BlockMu   sync.RWMutex
	v79NestBlock []HLSSegment
	v79StartSequence int
)
...
func buildFrozenVOD(session *StreamSession) (int, float64) {
	...
	log.Printf(
		"VERSION 79 FROZEN VOD READY: camera=%d name=%s generation=%d segments=%d duration=%.3f codec=[%s]",
		cameraIndex,
		cameraName,
		generation,
		len(copied),
...
func getCameras() ([]Camera, error) {
	...
	for attempt := 1; attempt <= 3; attempt++ {
		log.Printf(
			"VERSION 79 camera backend request attempt=%d url=%s/api/cameras",
			attempt,
			nestBackend,
		)
		...
		if err != nil {
			lastErr = err
			log.Printf(
				"VERSION 79 camera backend request failed attempt=%d error=%v",
				attempt,
				err,
			)
		} else {
			...
			if readErr != nil {
				lastErr = readErr
				log.Printf(
					"VERSION 79 camera backend read failed attempt=%d error=%v",
					attempt,
					readErr,
				)
				...
				log.Printf(
					"VERSION 79 camera backend HTTP failure attempt=%d status=%d body=%q",
					attempt,
					resp.StatusCode,
					string(responseBody),
				)
				...
				log.Printf(
					"VERSION 79 camera backend success attempt=%d status=%d bytes=%d",
					attempt,
					resp.StatusCode,
					len(body),
				)
...
func (s *StreamSession) inspectSliceOrderingLocked(
	...
	for _, nalu := range nalus {
		...
		if !diagnostic.Valid {
			...
			log.Printf(
				"VERSION 79 H264 SLICE PARSE ERROR: NAL=%d PTS=%d error=%s",
				nalType,
				pts,
				diagnostic.Error,
			)
		...
		if diagnostic.SliceType == "B" ||
			...
			log.Printf(
				"VERSION 79 H264 REORDER SIGNAL: slice=%s frameNum=%d POC=%d previousPOC=%d POCBackward=%t IDR=%t PTS=%d",
...
func (s *StreamSession) updateH264DiagnosticsLocked(
	...
	if !diagnostics.Valid {
		if hasSPS {
			log.Printf(
				"VERSION 79 H264 SPS PARSE ERROR: %s",
				diagnostics.Error,
			)
		}
	...
	if !s.haveActiveCodec {
		...
		log.Printf(
			"VERSION 79 H264 INITIAL CODEC: %s profileName=%s levelName=%s SPS=%d PPS=%d IDR=%t PTS=%d",
	...
	log.Printf(
		"VERSION 79 CODEC CHANGE DETECTED #%d: FROM [%s] TO [%s] IDR=%t PTS=%d",
		changeNumber,
	...
	if s.segmentActive &&
		...
		log.Printf(
			"VERSION 79 closing old codec segment before SPS transition: sequence=%d generation=%d",
			s.NextSequence,
			s.currentGeneration,
		)
	...
	log.Printf(
		"VERSION 79 HLS DISCONTINUITY ARMED: generation=%d nextSequence=%d",
		s.currentGeneration,
		s.NextSequence,
	)
}
...
func (s *StreamSession) normalizeTimestamp(
	...
	if !s.timestampStarted {
		...
		log.Printf(
			"VERSION 79 timestamp clock started: RTP=%d PTS=%d",
			rtpTimestamp,
			s.normalizedPTS,
		)
	...
	if delta > 90000*10 {
		...
		log.Printf(
			"VERSION 79 timestamp discontinuity: previous=%d current=%d rawDelta=%d",
			s.lastRTPTimestamp,
			rtpTimestamp,
			delta,
		)
...
func (s *StreamSession) newSegmentLocked() error {
	...
	log.Printf(
		"VERSION 79 SEGMENT OPEN: sequence=%d generation=%d codec=[%s] discontinuity=%t",
		s.NextSequence,
		s.currentGeneration,
...
func (s *StreamSession) finishSegmentLocked() {
	...
	if valid {
		...
		log.Printf(
			"VERSION 79 TS MEDIA VALID: packets=%d bytes=%d PAT=%d PMT=%d videoPID=%d streamType=0x%02x PES=%d PTS=%d DTS=%d PCR=%d continuityErrors=%d firstPTS=%d lastPTS=%d",
			diagnostics.Packets,
			diagnostics.Bytes,
		...
		log.Printf(
			"VERSION 79 TS MEDIA INVALID: error=%s packets=%d PAT=%d PMT=%d videoPID=%d streamType=0x%02x PES=%d PTS=%d backwardPTS=%d PCR=%d continuityErrors=%d transportErrors=%d",
			validateErr,
			diagnostics.Packets,
			diagnostics.PATPackets,
			diagnostics.PMTPackets,
	...
	log.Printf(
		"VERSION 79 HLS SEGMENT READY: sequence=%d generation=%d duration=%.3f size=%d mediaValid=%t discontinuityBefore=%t codec=[%s]",
		segment.Sequence,
		segment.Generation,
		segment.Duration,
		len(segment.Data),
...
func (s *StreamSession) keyframeAccessUnitLocked(
	...
	*/
	...
	if changed {
		log.Printf(
			"VERSION 79 H264 NORMALIZE: before=%s after=%s removedSPS=%d removedPPS=%d",
			before,
			after,
			duplicateSPS,
			duplicatePPS,
...
func (s *StreamSession) writeAccessUnit(
	...
	if !s.segmentActive {
		...
		log.Printf(
			"VERSION 79 HLS started on IDR: generation=%d SPS=%t PPS=%t PTS=%d NAL=%s codec=[%s]",
			s.currentGeneration,
			len(s.sps) > 0,
			len(s.pps) > 0,
			pts,
	...
	if hasIDR &&
		...
		log.Printf(
			"VERSION 79 new IDR segment: sequence=%d generation=%d PTS=%d NAL=%s",
			s.NextSequence,
			s.currentGeneration,
			pts,
			sourceSummary,
	...
	if hasIDR {
		log.Printf(
			"VERSION 79 MUX IDR: PTS=%d bytes=%d NAL=%s",
			pts,
			len(outputAU),
			s.lastMuxNALSummary,
		)
	...
	// VERSION 79: give Roku an explicit decode timestamp (DTS).
...
func createStreamSession(
	...
	pc.OnTrack(
		func(
			...
			log.Printf(
				"VERSION 79 incoming track: kind=%s codec=%s payload=%d",
				track.Kind().String(),
				codec.MimeType,
				codec.PayloadType,
			)
			...
			if track.Kind() == webrtc.RTPCodecTypeVideo {
				go func() {
					...
					for {
						...
						if err != nil {
							log.Printf(
								"VERSION 79 H264 depacketize error: %v",
								err,
							)
						...
						if packet.Marker &&
							...
							if err := session.writeAccessUnit(
								...
								log.Printf(
									"VERSION 79 MPEGTS write error: %v",
									err,
								)
							}
							...
							if units == 1 ||
								...
								log.Printf(
									"VERSION 79 H264 AU: units=%d packets=%d size=%d PTS=%d NAL=%s",
									units,
									packets,
									len(accessUnit),
									pts,
	...
	log.Println(
		"VERSION 79 SDP confirmed: audio -> video -> application",
	)
	...
	log.Printf(
		"VERSION 79 Nest backend HTTP status: %d",
		resp.StatusCode,
	)
	...
	log.Println(
		"VERSION 79 Nest WebRTC session started",
	)
...
func start(
	...
	log.Printf(
		"VERSION 79 start request parsed: camera=%d; requesting camera list",
		cameraIndex,
	)
	...
	if err != nil {
		log.Printf(
			"VERSION 79 start stopped at camera backend: %v",
			err,
		)
	...
	log.Printf(
		"VERSION 79 camera list ready: count=%d requested=%d",
		len(cameras),
		cameraIndex,
	)
	...
	log.Printf(
		"VERSION 79 starting camera %d: %s",
		cameraIndex,
		camera.Name,
	)
	...
	if err != nil {
		log.Println(
			"VERSION 79 camera start failed:",
			err,
		)
...
func snapshotLatestDebugSegment() (HLSSegment, int, error) {
	...
	log.Printf(
		"VERSION 79 DEBUG SEGMENT SNAPSHOT: camera=%d generation=%d sequence=%d bytes=%d",
		camera,
		segment.Generation,
		segment.Sequence,
		len(segment.Data),
...
func liveMasterPlaylistHandler(
	...
	log.Printf("VERSION 79 ROKU REQUEST STEP=DYNAMIC_HYBRID_PLAYLIST method=%s path=%s ua=%q remote=%s", r.Method, r.URL.Path, r.UserAgent(), r.RemoteAddr)

	// VERSION 79: wait only for a five-segment Nest startup cushion.
	...
	if session == nil {
		...
		http.Error(w, "waiting for Nest session", http.StatusServiceUnavailable)
		return
	}
	...
	for _, seg := range session.Segments {
		if seg.Validated && len(seg.Data) > 0 {
			nest = append(nest, copyHLSSegment(seg))
		}
	}
	session.mu.RUnlock()

	if len(nest) < 5 {
		log.Printf("VERSION 79 PLAYLIST GATE WAIT timedOut=true readySegments=%d", len(nest))
		w.Header().Set("Retry-After", "1")
		http.Error(w, "waiting for five validated Nest segments", http.StatusServiceUnavailable)
		return
	}
	log.Printf("VERSION 79 PLAYLIST GATE READY segments=%d firstSequence=%d lastSequence=%d", len(nest), nest[0].Sequence, nest[len(nest)-1].Sequence)

	// Keep the first eight entries of Apple's Roku-proven media playlist only as
	// a decoder bootstrap. Unlike V78, the playlist does NOT end with Apple's
	// finite 1:19 timeline. Live Nest entries are appended on every refresh.
	...
	if err != nil {
		http.Error(w, "apple master fetch failed", http.StatusBadGateway)
		return
	}
	defer resp.Body.Close()
	masterBody, err := io.ReadAll(resp.Body)
	if err != nil || resp.StatusCode != http.StatusOK {
		http.Error(w, "apple master unavailable", http.StatusBadGateway)
		return
	}
	...
	if child == "" {
		http.Error(w, "apple child not found", http.StatusBadGateway)
		return
	}
	childURL := child
	...
	if err != nil {
		http.Error(w, "apple child fetch failed", http.StatusBadGateway)
		return
	}
	defer childResp.Body.Close()
	childBody, err := io.ReadAll(childResp.Body)
	if err != nil || childResp.StatusCode != http.StatusOK {
		http.Error(w, "apple child unavailable", http.StatusBadGateway)
		return
	}
	childBase := childURL
	...
	appleMedia := 0
	for _, line := range strings.Split(string(childBody), "\n") {
		t := strings.TrimSpace(line)
		if t == "" {
			continue
		}
		if strings.HasPrefix(t, "#EXT-X-ENDLIST") {
			continue
		}
		if strings.HasPrefix(t, "#") {
			// Do not copy tags belonging to Apple segments after our bootstrap.
			if appleMedia < 8 {
				playlist.WriteString(t + "\n")
			}
			continue
		}
		if appleMedia >= 8 {
		...
		playlist.WriteString(fmt.Sprintf("https://nestview-roku-pion-v2.onrender.com/apple-segment?i=%d&u=%s\n", appleMedia, url.QueryEscape(absolute)))
		appleMedia++
	}
	...
	havePrevious := false
	for _, seg := range nest {
		if havePrevious && seg.Generation != previousGeneration {
			playlist.WriteString("#EXT-X-DISCONTINUITY\n")
			log.Printf("VERSION 79 DYNAMIC CODEC DISCONTINUITY sequence=%d fromGeneration=%d toGeneration=%d", seg.Sequence, previousGeneration, seg.Generation)
		}
		playlist.WriteString(fmt.Sprintf("#EXTINF:%.3f,\n", seg.Duration))
		playlist.WriteString(fmt.Sprintf("https://nestview-roku-pion-v2.onrender.com/live/segment%d.ts\n", seg.Sequence))
		previousGeneration = seg.Generation
		havePrevious = true
	}
	...
	log.Printf("VERSION 79 DYNAMIC HYBRID PLAYLIST appleSegments=%d nestSegments=%d firstNest=%d lastNest=%d bytes=%d", appleMedia, len(nest), nest[0].Sequence, nest[len(nest)-1].Sequence, len(body))
...
func appleSegmentTraceHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("VERSION 79 ROKU APPLE SEGMENT REQUEST index=%s target=%s ua=%q", idx, target, r.UserAgent())

	// VERSION 79: index 8 and every later Apple slot maps to the next Nest
	...
	if idxErr == nil && idxNum >= 8 {
		v79BlockMu.RLock()
		startSequence := v79StartSequence
		v79BlockMu.RUnlock()
		...
		for time.Now().Before(deadline) {
			...
			if session != nil {
				...
				if found != nil {
					...
					w.Header().Set("Cache-Control", "no-store")
					w.Header().Set("Content-Length", strconv.Itoa(len(body)))
					log.Printf("VERSION 79 CONTINUOUS NEST SEGMENT appleIndex=%d nestSequence=%d generation=%d duration=%.3f bytes=%d", idxNum, found.Sequence, found.Generation, found.Duration, len(body))
					_, _ = w.Write(body)
					return
				}
			}
		...
		log.Printf("VERSION 79 CONTINUOUS WAIT TIMEOUT appleIndex=%d nestSequence=%d", idxNum, targetSequence)
		w.Header().Set("Retry-After", "1")
	...
	w.Header().Set("Cache-Control", "no-store")
	w.Header().Set("Content-Length", strconv.Itoa(len(body)))
	log.Printf("VERSION 79 APPLE SEGMENT SERVED index=%s bytes=%d type=%q", idx, len(body), ct)
	_, _ = w.Write(body)
}
...
func livePlaylistHandler(
	...
	log.Printf("VERSION 79 ROKU REQUEST STEP=MEDIA method=%s path=%s query=%q ua=%q remote=%s", r.Method, r.URL.Path, r.URL.RawQuery, r.UserAgent(), r.RemoteAddr)
	session := getSession()
	...
	// VERSION 79: Roku startup buffer guard. Prefer the newest codec
	...
	log.Printf(
		"VERSION 79 ROKU STARTUP WINDOW: latestGeneration=%d latestCount=%d selectedGeneration=%d selectedCount=%d minimum=%d",
		latestGeneration,
	...
	// VERSION 79: Never advertise an undersized startup playlist to Roku.
	...
	if len(filtered) < rokuStartupSegments {
		log.Printf(
			"VERSION 79 ROKU MEDIA NOT READY: generation=%d ready=%d required=%d returning=503",
			selectedGeneration,
			len(filtered),
			rokuStartupSegments,
		)
	...
	if len(filtered) != len(segments) {
		log.Printf(
			"VERSION 79 LIVE PLAYLIST GENERATION FILTER: generation=%d kept=%d dropped=%d",
			selectedGeneration,
			len(filtered),
	...
	body := playlist.String()
	w.Header().Set("Content-Length", strconv.Itoa(len(body)))
	log.Printf("VERSION 79 ROKU MEDIA PLAYLIST BODY bytes=%d:\n%s", len(body), body)
...
func liveSegmentHandler(
	...
	log.Printf("VERSION 79 ROKU REQUEST STEP=SEGMENT method=%s path=%s query=%q range=%q ua=%q remote=%s", r.Method, r.URL.Path, r.URL.RawQuery, r.Header.Get("Range"), r.UserAgent(), r.RemoteAddr)
	session := getSession()
...
func main() {
	log.Println(
		"NestView TV Pion Roku bridge VERSION 79 starting",
	)

	log.Println(
		"VERSION 79: camera-backend retry and start-boundary diagnostics enabled",
	)
	...
	// VERSION 79: keep V56's proven HTTP redirect path, but redirect Roku
