...
var (
	...
	// V72: freeze a five-segment Nest handoff block before the Apple playlist
	// is exposed to Roku. This prevents Roku from reaching indexes 8-12
	// before the Nest media is actually ready.
	v72BlockMu   sync.RWMutex
	v72NestBlock []HLSSegment
)
...
func buildFrozenVOD(session *StreamSession) (int, float64) {
	...
	log.Printf(
		"VERSION 72 FROZEN VOD READY: camera=%d name=%s generation=%d segments=%d duration=%.3f codec=[%s]",
		cameraIndex,
		cameraName,
		generation,
		len(copied),
...
func getCameras() ([]Camera, error) {
	...
	for attempt := 1; attempt <= 3; attempt++ {
		log.Printf(
			"VERSION 72 camera backend request attempt=%d url=%s/api/cameras",
			attempt,
			nestBackend,
		)
		...
		if err != nil {
			lastErr = err
			log.Printf(
				"VERSION 72 camera backend request failed attempt=%d error=%v",
				attempt,
				err,
			)
		} else {
			...
			if readErr != nil {
				lastErr = readErr
				log.Printf(
					"VERSION 72 camera backend read failed attempt=%d error=%v",
					attempt,
					readErr,
				)
				...
				log.Printf(
					"VERSION 72 camera backend HTTP failure attempt=%d status=%d body=%q",
					attempt,
					resp.StatusCode,
					string(responseBody),
				)
				...
				log.Printf(
					"VERSION 72 camera backend success attempt=%d status=%d bytes=%d",
					attempt,
					resp.StatusCode,
					len(body),
				)
...
func (s *StreamSession) inspectSliceOrderingLocked(
	...
	for _, nalu := range nalus {
		...
		if !diagnostic.Valid {
			...
			log.Printf(
				"VERSION 72 H264 SLICE PARSE ERROR: NAL=%d PTS=%d error=%s",
				nalType,
				pts,
				diagnostic.Error,
			)
		...
		if diagnostic.SliceType == "B" ||
			...
			log.Printf(
				"VERSION 72 H264 REORDER SIGNAL: slice=%s frameNum=%d POC=%d previousPOC=%d POCBackward=%t IDR=%t PTS=%d",
...
func (s *StreamSession) updateH264DiagnosticsLocked(
	...
	if !diagnostics.Valid {
		if hasSPS {
			log.Printf(
				"VERSION 72 H264 SPS PARSE ERROR: %s",
				diagnostics.Error,
			)
		}
	...
	if !s.haveActiveCodec {
		...
		log.Printf(
			"VERSION 72 H264 INITIAL CODEC: %s profileName=%s levelName=%s SPS=%d PPS=%d IDR=%t PTS=%d",
	...
	log.Printf(
		"VERSION 72 CODEC CHANGE DETECTED #%d: FROM [%s] TO [%s] IDR=%t PTS=%d",
		changeNumber,
	...
	if s.segmentActive &&
		...
		log.Printf(
			"VERSION 72 closing old codec segment before SPS transition: sequence=%d generation=%d",
			s.NextSequence,
			s.currentGeneration,
		)
	...
	log.Printf(
		"VERSION 72 HLS DISCONTINUITY ARMED: generation=%d nextSequence=%d",
		s.currentGeneration,
		s.NextSequence,
	)
}
...
func (s *StreamSession) normalizeTimestamp(
	...
	if !s.timestampStarted {
		...
		log.Printf(
			"VERSION 72 timestamp clock started: RTP=%d PTS=%d",
			rtpTimestamp,
			s.normalizedPTS,
		)
	...
	if delta > 90000*10 {
		...
		log.Printf(
			"VERSION 72 timestamp discontinuity: previous=%d current=%d rawDelta=%d",
			s.lastRTPTimestamp,
			rtpTimestamp,
			delta,
		)
...
func (s *StreamSession) newSegmentLocked() error {
	...
	log.Printf(
		"VERSION 72 SEGMENT OPEN: sequence=%d generation=%d codec=[%s] discontinuity=%t",
		s.NextSequence,
		s.currentGeneration,
...
func (s *StreamSession) finishSegmentLocked() {
	...
	if valid {
		...
		log.Printf(
			"VERSION 72 TS MEDIA VALID: packets=%d bytes=%d PAT=%d PMT=%d videoPID=%d streamType=0x%02x PES=%d PTS=%d DTS=%d PCR=%d continuityErrors=%d firstPTS=%d lastPTS=%d",
			diagnostics.Packets,
			diagnostics.Bytes,
		...
		log.Printf(
			"VERSION 72 TS MEDIA INVALID: error=%s packets=%d PAT=%d PMT=%d videoPID=%d streamType=0x%02x PES=%d PTS=%d backwardPTS=%d PCR=%d continuityErrors=%d transportErrors=%d",
			validateErr,
			diagnostics.Packets,
			diagnostics.PATPackets,
			diagnostics.PMTPackets,
	...
	log.Printf(
		"VERSION 72 HLS SEGMENT READY: sequence=%d generation=%d duration=%.3f size=%d mediaValid=%t discontinuityBefore=%t codec=[%s]",
		segment.Sequence,
		segment.Generation,
		segment.Duration,
		len(segment.Data),
...
func (s *StreamSession) keyframeAccessUnitLocked(
	...
	*/
	...
	if changed {
		log.Printf(
			"VERSION 72 H264 NORMALIZE: before=%s after=%s removedSPS=%d removedPPS=%d",
			before,
			after,
			duplicateSPS,
			duplicatePPS,
...
func (s *StreamSession) writeAccessUnit(
	...
	if !s.segmentActive {
		...
		log.Printf(
			"VERSION 72 HLS started on IDR: generation=%d SPS=%t PPS=%t PTS=%d NAL=%s codec=[%s]",
			s.currentGeneration,
			len(s.sps) > 0,
			len(s.pps) > 0,
			pts,
	...
	if hasIDR &&
		...
		log.Printf(
			"VERSION 72 new IDR segment: sequence=%d generation=%d PTS=%d NAL=%s",
			s.NextSequence,
			s.currentGeneration,
			pts,
			sourceSummary,
	...
	if hasIDR {
		log.Printf(
			"VERSION 72 MUX IDR: PTS=%d bytes=%d NAL=%s",
			pts,
			len(outputAU),
			s.lastMuxNALSummary,
		)
	...
	// VERSION 72: give Roku an explicit decode timestamp (DTS).
...
func createStreamSession(
	...
	pc.OnTrack(
		func(
			...
			log.Printf(
				"VERSION 72 incoming track: kind=%s codec=%s payload=%d",
				track.Kind().String(),
				codec.MimeType,
				codec.PayloadType,
			)
			...
			if track.Kind() == webrtc.RTPCodecTypeVideo {
				go func() {
					...
					for {
						...
						if err != nil {
							log.Printf(
								"VERSION 72 H264 depacketize error: %v",
								err,
							)
						...
						if packet.Marker &&
							...
							if err := session.writeAccessUnit(
								...
								log.Printf(
									"VERSION 72 MPEGTS write error: %v",
									err,
								)
							}
							...
							if units == 1 ||
								...
								log.Printf(
									"VERSION 72 H264 AU: units=%d packets=%d size=%d PTS=%d NAL=%s",
									units,
									packets,
									len(accessUnit),
									pts,
	...
	log.Println(
		"VERSION 72 SDP confirmed: audio -> video -> application",
	)
	...
	log.Printf(
		"VERSION 72 Nest backend HTTP status: %d",
		resp.StatusCode,
	)
	...
	log.Println(
		"VERSION 72 Nest WebRTC session started",
	)
...
func start(
	...
	log.Printf(
		"VERSION 72 start request parsed: camera=%d; requesting camera list",
		cameraIndex,
	)
	...
	if err != nil {
		log.Printf(
			"VERSION 72 start stopped at camera backend: %v",
			err,
		)
	...
	log.Printf(
		"VERSION 72 camera list ready: count=%d requested=%d",
		len(cameras),
		cameraIndex,
	)
	...
	log.Printf(
		"VERSION 72 starting camera %d: %s",
		cameraIndex,
		camera.Name,
	)
	...
	if err != nil {
		log.Println(
			"VERSION 72 camera start failed:",
			err,
		)
...
func snapshotLatestDebugSegment() (HLSSegment, int, error) {
	...
	log.Printf(
		"VERSION 72 DEBUG SEGMENT SNAPSHOT: camera=%d generation=%d sequence=%d bytes=%d",
		camera,
		segment.Generation,
		segment.Sequence,
		len(segment.Data),
...
func liveMasterPlaylistHandler(
	...
	log.Printf("VERSION 72 ROKU REQUEST STEP=APPLE_SEGMENT_TRACE method=%s path=%s ua=%q remote=%s", r.Method, r.URL.Path, r.UserAgent(), r.RemoteAddr)

	// V72 readiness gate: do not give Roku the Apple/Nest hybrid playlist until
	// five validated Nest segments from one codec generation are already frozen.
	// Roku reaches the handoff very quickly, so waiting here prevents 503s at
	// Apple indexes 8-12.
	...
	for time.Now().Before(deadline) {
		...
		if session != nil {
			...
			for pos := len(all) - 1; pos >= 0 && len(readyBlock) == 0; pos-- {
				gen := all[pos].Generation
				var group []HLSSegment
				for _, seg := range all {
					if seg.Generation == gen && seg.Validated && len(seg.Data) > 0 {
						group = append(group, copyHLSSegment(seg))
					}
				}
				if len(group) >= 5 {
					readyBlock = group[len(group)-5:]
					readyGeneration = gen
				}
				for pos > 0 && all[pos-1].Generation == gen {
					pos--
				}
			}
		}
	...
	if len(readyBlock) < 5 {
		log.Printf("VERSION 72 PLAYLIST GATE WAIT timedOut=true readySegments=%d", len(readyBlock))
	...
	v72BlockMu.Lock()
	v72NestBlock = make([]HLSSegment, len(readyBlock))
	for i := range readyBlock {
		v72NestBlock[i] = copyHLSSegment(readyBlock[i])
	}
	v72BlockMu.Unlock()
	log.Printf("VERSION 72 PLAYLIST GATE READY generation=%d sequences=%d,%d,%d,%d,%d", readyGeneration, readyBlock[0].Sequence, readyBlock[1].Sequence, readyBlock[2].Sequence, readyBlock[3].Sequence, readyBlock[4].Sequence)
	...
	lines := strings.Split(string(childBody), "\n")
	mediaIndex := 0
	for i, line := range lines {
		...
		lines[i] = fmt.Sprintf("https://nestview-roku-pion-v2.onrender.com/apple-segment?i=%d&u=%s", mediaIndex, url.QueryEscape(absolute))
		mediaIndex++
	}
	...
	log.Printf("VERSION 72 APPLE SEGMENT TRACE PLAYLIST child=%s mediaURIs=%d bytes=%d", childURL, mediaIndex, len(out))
...
func appleSegmentTraceHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("VERSION 72 ROKU APPLE SEGMENT REQUEST index=%s target=%s ua=%q", idx, target, r.UserAgent())

	// V72: indexes 8-12 are served from the five-segment block frozen before
	// the playlist was returned. There is no race with live segment production.
	idxNum, idxErr := strconv.Atoi(idx)
	if idxErr == nil && idxNum >= 8 && idxNum <= 12 {
		v72BlockMu.RLock()
		block := make([]HLSSegment, len(v72NestBlock))
		for i := range v72NestBlock {
			block[i] = copyHLSSegment(v72NestBlock[i])
		}
		v72BlockMu.RUnlock()

		offset := idxNum - 8
		if len(block) != 5 || offset < 0 || offset >= len(block) {
			log.Printf("VERSION 72 FROZEN BLOCK MISSING appleIndex=%d blockSize=%d", idxNum, len(block))
		...
		log.Printf("VERSION 72 FORCED NEST BLOCK appleIndex=%d blockOffset=%d nestSequence=%d generation=%d duration=%.3f bytes=%d", idxNum, offset, seg.Sequence, seg.Generation, seg.Duration, len(body))
		_, _ = w.Write(body)
		return
	}
	...
	w.Header().Set("Cache-Control", "no-store")
	w.Header().Set("Content-Length", strconv.Itoa(len(body)))
	log.Printf("VERSION 72 APPLE SEGMENT SERVED index=%s bytes=%d type=%q", idx, len(body), ct)
	_, _ = w.Write(body)
}
func livePlaylistHandler(
	...
	log.Printf("VERSION 72 ROKU REQUEST STEP=MEDIA method=%s path=%s query=%q ua=%q remote=%s", r.Method, r.URL.Path, r.URL.RawQuery, r.UserAgent(), r.RemoteAddr)
	session := getSession()
	...
	// VERSION 72: Roku startup buffer guard. Prefer the newest codec
	...
	log.Printf(
		"VERSION 72 ROKU STARTUP WINDOW: latestGeneration=%d latestCount=%d selectedGeneration=%d selectedCount=%d minimum=%d",
		latestGeneration,
	...
	// VERSION 72: Never advertise an undersized startup playlist to Roku.
	...
	if len(filtered) < rokuStartupSegments {
		log.Printf(
			"VERSION 72 ROKU MEDIA NOT READY: generation=%d ready=%d required=%d returning=503",
			selectedGeneration,
			len(filtered),
			rokuStartupSegments,
		)
	...
	if len(filtered) != len(segments) {
		log.Printf(
			"VERSION 72 LIVE PLAYLIST GENERATION FILTER: generation=%d kept=%d dropped=%d",
			selectedGeneration,
			len(filtered),
	...
	w.Header().Set("Content-Length", strconv.Itoa(len(body)))
	log.Printf("VERSION 72 ROKU MEDIA PLAYLIST BODY bytes=%d:\n%s", len(body), body)
...
func liveSegmentHandler(
	...
	log.Printf("VERSION 72 ROKU REQUEST STEP=SEGMENT method=%s path=%s query=%q range=%q ua=%q remote=%s", r.Method, r.URL.Path, r.URL.RawQuery, r.Header.Get("Range"), r.UserAgent(), r.RemoteAddr)
	session := getSession()
...
func main() {
	log.Println(
		"NestView TV Pion Roku bridge VERSION 72 starting",
	)

	log.Println(
		"VERSION 72: camera-backend retry and start-boundary diagnostics enabled",
	)
	...
	// VERSION 72: keep V56's proven HTTP redirect path, but redirect Roku
